...
/** * NODE PROPERTIES:
  ...
  * node. The offset is such that parts of the content can easily be accessed via `content.substring(offset,
  * offsetEnd)`. This means that the offset must be measured in utf16 encoding (i.e. neither in characters/codeunits nor
  * in byte-offsets into a utf8 encoding). E.g. for METHOD nodes this start offset points to the start of the methods
...
class Unknown(graph_4762: flatgraph.Graph, seq_4762: Int)
    extends StoredNode(graph_4762, 43, seq_4762)
    with UnknownBase
    with Expression
