...
object Accessors {
  ...
  final class AccessPropertyMyproperty(val node: nodes.StoredNode) extends AnyVal {
    def myproperty: String = flatgraph.Accessors.getNodePropertySingle(
      node.graph,
      nodeKind = node.nodeKind,
      propertyKind = 37,
      seq = node.seq(),
  ...
  final class AccessMynodetypeBase(val node: nodes.MynodetypeBase) extends AnyVal {
    def myproperty: String = node match {
      case stored: nodes.StoredNode     => new AccessPropertyMyproperty(stored).myproperty
      case newNode: nodes.NewMynodetype => newNode.myproperty
    }
  }
...
trait ConcreteStoredConversions extends ConcreteBaseConversions {
  ...
  implicit def accessPropertyMyproperty(
    node: nodes.StoredNode & nodes.StaticType[nodes.HasMypropertyEMT]
  ): AccessPropertyMyproperty = new AccessPropertyMyproperty(node)
...
trait ConcreteBaseConversions extends AbstractBaseConversions0 {
  ...
  implicit def accessMynodetypebase(node: nodes.MynodetypeBase): AccessMynodetypeBase = new AccessMynodetypeBase(node)
